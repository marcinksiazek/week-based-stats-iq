...
class WeekStatUtils {
    ...
    static function getCurrentWeekData() as {  :weekNumber as Number,
        ...
        var dayOfYear = (now.compare(startOfYear) / Gregorian.SECONDS_PER_DAY).toNumber() + 1;
        
        var weekNumber = ((dayOfYear - today.day_of_week + 10) / 7).toNumber();
        var startOfWeek = now.subtract(new Time.Duration(dayOfWeek * Gregorian.SECONDS_PER_DAY));
        var endOfWeek = startOfWeek.add(new Time.Duration(6 * Gregorian.SECONDS_PER_DAY));
    ...
    static function formatDateRange(startTime as Moment, endTime as Moment) as String{
    ...
    static function getActivityTotals(startDate as Moment, endDate as Moment)
                        as {    :running as { :distance as Number, :count as Number },
